...
function BossImage({ src, alt }: { src?: string; alt?: string }) {
    ...
    return status === "error" || !fixedSrc ? (
        <div className="w-full rounded-xl bg-[#7A3A8E]/50 border border-dashed border-[#7A3A8E] p-4 text-center my-3">
            <ImageIcon className="w-8 h-8 text-[#AD74C3] mx-auto mb-2" />
            <p className="text-[#AD74C3] text-xs italic">{alt || "Ilustración"}</p>
        </div>
    ) : (
        <div className="my-3 relative">
            {status === "loading" && (
                <div className="absolute inset-0 flex items-center justify-center bg-[#7A3A8E]/50 rounded-xl animate-pulse">
            ...
            <img
                src={fixedSrc}
                alt={alt || "Ilustración"}
                className="w-full rounded-xl border border-[#7A3A8E]"
                loading="lazy"
...
export default function BossFightCamera({ data, studentName = "Aventurero", studentId, worldId, levelId, onComplete, onClose }: BossFightCameraProps) {
    ...
    return (
        <div className="fixed inset-0 bg-[#522566]/95 flex flex-col items-center justify-center p-4 z-50 backdrop-blur-md">
            ...
            <div className="w-full max-w-lg bg-red-950/50 border-b border-red-500/30 p-4 flex justify-between items-center rounded-t-3xl">
                ...
                <div className="flex gap-2 items-center">
                    ...
                    <button onClick={onClose} className="rounded-full p-2 bg-[#522566] text-[#AD74C3] hover:text-white">
                        <X />
                    </button>
                </div>
            </div>
            ...
            {showTeacherAuth && (
                <div className="w-full max-w-lg bg-[#522566] p-4 text-white flex gap-3 items-center justify-center border-x border-[#7A3A8E] animate-fade-in-up">
            ...
            <div className={`w-full max-w-lg bg-[#522566] border-x border-b border-[#7A3A8E] rounded-b-3xl overflow-hidden flex flex-col ${showTeacherAuth ? 'max-h-[75vh]' : 'max-h-[85vh]'}`}>

                {/* Problem Statement Area */}
                {step !== 'camera' && step !== 'preview' && (
                    <div className="p-6 bg-[#522566] border-b border-[#7A3A8E] max-h-[45vh] overflow-y-auto">
                        <div className="bg-[#7A3A8E]/50 p-4 rounded-xl border border-[#7A3A8E] text-slate-200 leading-relaxed prose prose-invert prose-sm max-w-none">
                ...
                <div className="flex-1 p-6 flex flex-col items-center justify-center min-h-[300px]">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {(requiredEvidenceType !== "TEXTO_ENSAYO") && (
                                    <button
                                        ...
                                        className="flex flex-col items-center justify-center p-6 bg-[#522566] rounded-3xl border-4 border-dashed border-red-500/50 hover:bg-[#7A3A8E] hover:border-red-400 transition-colors group"
                                    >
                                ...
                                {(requiredEvidenceType !== "FOTO_DIBUJO" && requiredEvidenceType !== "FOTO_GRAFICA") && (
                                    <button
                                        ...
                                        className="flex flex-col items-center justify-center p-6 bg-[#522566] rounded-3xl border-4 border-dashed border-amber-500/50 hover:bg-[#7A3A8E] hover:border-amber-400 transition-colors group"
                                    >
                    ...
                    {step === "text_input" && (
                        <div className="w-full flex md:flex-col flex-col gap-4">
                            ...
                            <textarea
                                className="w-full p-4 rounded-xl border border-[#7A3A8E] bg-[#522566] text-white focus:border-red-500 outline-none resize-none h-32"
                                placeholder="Ejemplo: La respuesta es 45 metros cuadrados..."
                                value={textEvidence}
                                onChange={e => setTextEvidence(e.target.value)}
                            />
                            <div className="flex gap-3">
                                <button onClick={() => setStep("idle")} className="flex-1 py-3 bg-[#7A3A8E] text-white rounded-xl font-bold">Atrás</button>
                                <button
                    ...
                    {step === 'preview' && imagePreview && (
                        <div className="w-full h-full flex flex-col">
                            <div className="relative flex-1 rounded-xl overflow-hidden border border-[#7A3A8E] bg-black">
                            ...
                            <div className="flex gap-4 mt-6">
                                <button onClick={() => { setStep('idle'); setImagePreview(null); }} className="flex-1 py-3 bg-[#7A3A8E] text-white rounded-xl font-bold">
                                    Repetir
                                </button>
                    ...
                    {step === 'analyzing' && (
                        <div className="space-y-6 text-center">
                            <div className="relative w-32 h-32 mx-auto">
                                ...
                                <div className="absolute inset-2 border-b-4 border-[#7A3A8E] rounded-full animate-spin-reverse"></div>
                            </div>
                            <h3 className="text-xl text-white font-bold animate-pulse">Analizando Lógica...</h3>
                            <div className="space-y-1 text-sm text-[#AD74C3]">
                        ...
                        <div className="w-full space-y-5 overflow-y-auto max-h-[60vh] pr-1">
                            ...
                            {feedback.correct ? (
                                <div className="text-center space-y-3">
                                    ...
                                    <p className="text-[#AD74C3] text-sm">
                                        ¡Felicidades, <strong className="text-white">{studentName}</strong>! Has demostrado dominio total del tema.
                                    </p>
                                </div>
                            ) : feedback.canAdvance ? (
                                <div className="text-center space-y-3">
                                    <div className={`p-1 rounded-full w-20 h-20 mx-auto flex items-center justify-center bg-[#7A3A8E]/20 text-[#7A3A8E]`}>
                                        <Sparkles className="w-10 h-10" />
                                    </div>
                                    <h3 className="text-xl font-bold text-[#AD74C3]">¡Victoria Parcial!</h3>
                                    <p className="text-[#AD74C3] text-sm">El jefe está débil, pero puedes mejorar tu ataque (Sacar 10) si corriges tu error.</p>
                                </div>
                            ) : (
                                <div className="text-center space-y-3">
                                    ...
                                    <p className="text-[#AD74C3] text-sm">No te rindas, revisa tu respuesta e intenta de nuevo.</p>
                                </div>
                            )}
                            ...
                            <div className={`mx-auto w-28 h-28 rounded-full flex flex-col items-center justify-center border-4 ${feedback.grade >= 8 ? 'border-green-500 bg-green-500/10' :
                                ...
                                <span className="text-xs text-[#AD74C3] font-bold">/10</span>
                            </div>

                            {/* AI Feedback */}
                            <div className="bg-[#522566] rounded-xl p-5 border border-[#7A3A8E]">
                                <h4 className="text-[#AD74C3] text-xs uppercase font-bold text-center mb-3">🤖 Retroalimentación de la IA</h4>
                                <ul className="space-y-3">
                                    {feedback.hints.map((hint, i) => (
                                        <li key={i} className="flex gap-3 text-sm text-slate-200 leading-relaxed">
                                            <span className="text-[#AD74C3] flex-shrink-0">•</span>
                            ...
                            {feedback.correct && (
                                <div className="bg-gradient-to-r from-red-950/80 to-purple-950/80 rounded-xl p-4 border border-red-500/30 text-center space-y-2">
                                    ...
                                    <p className="text-[#AD74C3] text-xs leading-relaxed">
                            ...
                            <div className="flex flex-col gap-3">
                                {!feedback.canAdvance ? (
                                    <button onClick={() => { setFeedback(null); setStep("text_input"); }} className="w-full py-4 font-bold rounded-xl transition-colors bg-[#7A3A8E] hover:bg-slate-600 text-white flex items-center justify-center gap-2">
                                        <RefreshCw className="w-5 h-5" /> Intentar de nuevo
                                    </button>
                                ) : (
                                    <>
                                        {(!feedback.correct && feedback.canAdvance) && (
                                            <button onClick={() => { setFeedback(null); setStep("text_input"); }} className="w-full py-4 font-bold rounded-xl transition-colors border-2 border-blue-500/50 text-[#AD74C3] hover:bg-blue-900/30 flex items-center justify-center gap-2">
