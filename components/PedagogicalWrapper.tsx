...
const markdownComponents: any = {
    code({ node, inline, className, children, ...props }: any) {
        return (
            <code className="bg-[#c1ebd5] dark:bg-[#0a2d1d] rounded px-1 py-0.5 text-[#0a2d1d] dark:text-[#2e9f6c] font-bold" {...props}>
                {children}
            </code>
        );
    },
    a({ node, children, href, ...props }: any) {
        return <a href={href} target="_blank" rel="noopener noreferrer" className="text-[#0a2d1d] hover:text-sky-800 underline decoration-2 decoration-sky-300 underline-offset-2" {...props}>{children}</a>;
    },
    blockquote({ node, children, ...props }: any) {
        return <blockquote className="border-l-4 border-amber-500 bg-amber-50 dark:bg-amber-900/20 p-4 rounded-r-xl italic my-4 text-[#165b3d] dark:text-[#2e9f6c]" {...props}>{children}</blockquote>;
    },
    ul({ node, children, ...props }: any) {
        return <ul className="list-disc pl-6 space-y-2 my-4" {...props}>{children}</ul>;
    },
    ol({ node, children, ...props }: any) {
        return <ol className="list-decimal pl-6 space-y-2 my-4 font-bold text-[#165b3d] dark:text-[#2e9f6c]" {...props}>{children}</ol>;
    },
};
...
const renderSafeContent = (content: any) => {
    if (typeof content === 'object' && content !== null) {
        return <pre className="whitespace-pre-wrap text-sm bg-[#c1ebd5] text-[#0a2d1d] p-4 rounded-xl overflow-x-auto my-4 border border-[#c1ebd5] shadow-inner max-w-full">{JSON.stringify(content, null, 2)}</pre>;
    }
    return (
...
const FlashcardView = ({ paragraphs }: { paragraphs: string[] }) => {
    ...
    return (
        <div className="flex flex-col items-center w-full max-w-2xl mx-auto space-y-6">
            <div className="w-full bg-white dark:bg-[#0a2d1d] p-8 rounded-3xl shadow-lg border-b-4 border-[#c1ebd5] dark:border-sky-900 min-h-[250px] flex items-center justify-center relative overflow-hidden transition-all duration-300 hover:shadow-xl">
                <div className="absolute top-0 right-0 bg-[#c1ebd5] dark:bg-sky-900/50 text-sky-800 dark:text-[#c1ebd5] px-3 py-1 rounded-bl-xl font-bold text-sm">
            ...
            <div className="flex gap-4 w-full justify-between">
                <button
                    ...
                    className="flex items-center gap-2 px-4 py-2 bg-[#c1ebd5] hover:bg-[#c1ebd5] dark:bg-[#165b3d] dark:hover:bg-slate-600 rounded-full font-bold text-[#165b3d] dark:text-[#2e9f6c] disabled:opacity-50 transition-colors"
                >
                    <ChevronLeft className="w-5 h-5" /> Anterior
                </button>
                <div className="flex gap-1 items-center">
                    {paragraphs.map((_, i) => (
                        <div key={i} className={`h-2 rounded-full transition-all ${i === currentIndex ? 'w-6 bg-[#165b3d]' : 'w-2 bg-slate-300 dark:bg-slate-600'}`} />
                    ))}
                </div>
                <button
                    onClick={() => setCurrentIndex(p => Math.min(paragraphs.length - 1, p + 1))}
                    disabled={currentIndex === paragraphs.length - 1}
                    className="flex items-center gap-2 px-4 py-2 bg-[#c1ebd5] hover:bg-[#c1ebd5] dark:bg-sky-900/50 dark:hover:bg-sky-800 rounded-full font-bold text-sky-700 dark:text-[#2e9f6c] disabled:opacity-50 transition-colors"
                >
                        ...
                        <div className={`flex max-w-[85%] gap-3 ${isStudent ? 'flex-row-reverse' : 'flex-row'}`}>
                            ...
                            <div className="flex-shrink-0 mt-1">
                                {isStudent ? (
                                    ...
                                    <div className="w-10 h-10 rounded-full bg-[#c1ebd5] dark:bg-sky-900/50 border-2 border-[#165b3d] flex items-center justify-center shadow-lg shadow-[#c1ebd5]">
                                        <Bot className="w-6 h-6 text-[#0a2d1d] dark:text-[#2e9f6c]" />
                                    </div>
                                )}
                            </div>
                            ...
                            <div className={`p-4 rounded-2xl shadow-sm min-w-0 overflow-hidden break-words ${isStudent
                                ...
                                : 'bg-white dark:bg-[#0a2d1d] border border-[#c1ebd5] dark:border-[#165b3d] rounded-tl-sm shadow-md'
                                }`}>
    ...
    return (
        <div className="w-full max-w-2xl mx-auto space-y-3">
            {paragraphs.map((paragraph, index) => {
                ...
                return (
                    <div key={index} className={`border-2 rounded-2xl overflow-hidden transition-all duration-300 ${isOpen ? 'border-[#2e9f6c] dark:border-sky-600 shadow-md transform scale-[1.02]' : 'border-[#c1ebd5] dark:border-[#165b3d] hover:border-sky-300'}`}>
                        <button
                            onClick={() => setOpenIndex(isOpen ? -1 : index)}
                            className={`w-full flex items-center justify-between p-4 font-bold text-left transition-colors ${isOpen ? 'bg-[#f0fbf5] dark:bg-sky-900/30 text-sky-800 dark:text-[#c1ebd5]' : 'bg-white dark:bg-[#0a2d1d] text-[#165b3d] dark:text-[#2e9f6c] hover:bg-[#f0fbf5]'}`}
                        >
                            <span className="flex items-center gap-3">
                                <span className={`flex items-center justify-center w-8 h-8 rounded-full text-sm ${isOpen ? 'bg-[#0a2d1d] text-white' : 'bg-[#c1ebd5] dark:bg-[#165b3d] text-[#165b3d] dark:text-[#2e9f6c]'}`}>
                                    {index + 1}
                                </span>
                                {title}
                            </span>
                            <ChevronDown className={`w-5 h-5 transition-transform duration-300 ${isOpen ? 'rotate-180 text-[#0a2d1d]' : 'text-[#2e9f6c]'}`} />
                        </button>

                        <div className={`overflow-hidden transition-all duration-300 ease-in-out ${isOpen ? 'max-h-[1000px] opacity-100' : 'max-h-0 opacity-0'}`}>
                            <div className="p-6 bg-white dark:bg-[#0a2d1d] prose prose-lg dark:prose-invert max-w-full break-words border-t border-[#c1ebd5] dark:border-[#165b3d]/50">
                                {renderSafeContent(paragraph)}
                            </div>
                        </div>
                    </div>
...
export default function PedagogicalWrapper({ content, studentName, type = 'theory' }: PedagogicalWrapperProps) {
    ...
    if (rawParagraphs.length <= 1 || hasMarkdownHeaders) {
        return (
            <div className={`bg-white dark:bg-[#0a2d1d] p-6 rounded-2xl shadow-sm border ${type === 'narrative' ? 'border-amber-200 bg-amber-50/50' : 'border-[#c1ebd5]'} overflow-hidden break-words`}>
