...
const renderSafeContent = (content: any) => {
    if (typeof content === 'object' && content !== null) {
        return <pre className="whitespace-pre-wrap text-sm bg-gray-100 dark:bg-[#1c3a60] text-[#1c3a60] dark:text-slate-200 p-4 rounded-xl overflow-x-auto my-4 border border-[#cbe0f6] shadow-inner max-w-full">{JSON.stringify(content, null, 2)}</pre>;
    }
    return (
...
export default function InteractiveLessonCard({ data, studentName = "Aventurero", studentId, worldId, levelId, onComplete, onClose }: InteractiveLessonCardProps) {
    ...
    const renderGuidedPractice = () => {
        ...
        return (
            <div className="space-y-6 animate-fade-in-up">
                <div className="bg-white dark:bg-[#1c3a60] p-6 rounded-2xl shadow-sm border border-teal-100 dark:border-[#346297]">
                    <div className="bg-[#f0f5fb] dark:bg-[#346297] p-6 rounded-xl border border-teal-100 dark:border-[#346297]">
                        ...
                        {speechSupported && (
                            <div className="mt-4 flex justify-end">
                                <button
                                    ...
                                    className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-sm font-bold transition-colors ${isSpeaking ? 'bg-teal-200 text-teal-700 animate-pulse' : 'bg-[#cbe0f6] text-[#1c3a60] hover:bg-teal-200'}`}
                ...
                {(() => {
                    ...
                    return (
                        <div className="bg-[#f0f5fb] dark:bg-blue-900/30 border-l-4 border-blue-500 p-6 rounded-r-xl shadow-sm my-6">
                ...
                <div className="flex gap-4 mt-6">
                    <button
                        type="button"
                        onClick={onComplete}
                        className="flex-1 bg-[#1c3a60] hover:bg-[#1c3a60] text-white py-4 rounded-xl font-bold text-lg shadow-lg shadow-teal-600/30 transition-transform active:scale-95"
                    >
                        📝 Subir Evidencia
                    </button>
                    <button
    ...
    const renderMiniGame = () => {
        ...
        return (
            <div className="space-y-6 animate-fade-in-up">
                <h2 className="text-2xl font-bold text-center text-[#1c3a60] mb-6 font-display">
                    ¡Mini-Desafío! 🧠
                </h2>

                <div className="bg-white p-6 rounded-2xl shadow-md border-2 border-teal-100 text-center w-full max-w-full">
                    <p className="text-xl text-[#346297] mb-8 break-words whitespace-pre-wrap w-full max-w-full overflow-hidden relative">
                        ...
                        {(data.content?.miniGame?.options || []).map((option, idx) => (
                            ...
                            ${feedback === 'success' && option === data.content?.miniGame?.correctAnswer
                                        ...
                                        : disabledOptions[option]
                                            ? 'bg-[#cbe0f6] border-[#cbe0f6] text-[#73a4db] opacity-60 cursor-not-allowed'
                                            : 'bg-[#f0f5fb] border-teal-200 text-teal-700 hover:bg-[#cbe0f6] hover:border-teal-400'}
    ...
    return (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50 backdrop-blur-sm">
            ...
            {showGameOver && (
                <div className="fixed inset-0 z-[60] bg-black/90 flex items-center justify-center backdrop-blur-lg">
                    <div className="text-center animate-fade-in-up">
                        ...
                        <p className="text-[#73a4db] text-lg mb-2">¡Se acabaron tus vidas!</p>
                        <p className="text-[#73a4db] text-sm mb-8">Tus vidas se recargarán automáticamente...</p>
            ...
            <div className="bg-white dark:bg-[#1c3a60] rounded-3xl w-full max-w-2xl overflow-hidden shadow-2xl border-4 border-amber-200 dark:border-[#346297] flex flex-col max-h-[90vh]">
                <div className="bg-amber-100 dark:bg-[#1c3a60] p-3 sm:p-4 flex justify-between items-start border-b border-amber-200 dark:border-[#346297] gap-2">
                    ...
                    <div className="flex gap-2 items-center">
                        ...
                        <div className="flex items-center gap-2 sm:gap-3 mr-1 sm:mr-2 bg-white/60 dark:bg-[#346297]/60 px-2 sm:px-3 py-1 rounded-full">
                            <div className="flex gap-0.5 sm:gap-1 animate-fade-in-up">
                                ...
                                {Array.from({ length: Math.max(0, 3 - stats.lives) }).map((_, i) => (
                                    <Heart key={`empty-${i}`} className="w-5 h-5 sm:w-8 sm:h-8 md:w-10 md:h-10 text-[#73a4db] fill-slate-200" />
                                ))}
                            </div>
                            <div className="hidden sm:flex items-center gap-0.5">
                                <Diamond className="w-4 h-4 text-[#346297] fill-blue-400" />
                                <span className="text-xs font-bold text-[#1c3a60]">{stats.gems}</span>
                            </div>
                            <div className="hidden sm:flex items-center gap-0.5">
                        ...
                        {(data.type === 'guided_practice' || data.content?.miniGame) && showActivity && !isTeacherUnlocked && (
                            <button type="button" onClick={() => setShowTeacherAuth(!showTeacherAuth)} className="text-[#73a4db] hover:text-[#346297] bg-white/50 px-3 py-1 rounded-full text-xs font-bold transition-colors">
                                👁️ Docente
                            </button>
                        )}
                ...
                {showTeacherAuth && !isTeacherUnlocked && (
                    <div className="bg-[#1c3a60] p-4 text-white flex gap-3 items-center justify-center animate-fade-in-up">
                        <span className="text-sm font-bold text-[#73a4db]">Contraseña Docente:</span>
                        <input
                            type="password"
                            className="text-black px-3 py-1.5 rounded-lg text-sm w-32 outline-none focus:ring-2 focus:ring-[#73a4db]"
                            value={teacherPassword}
                            onChange={e => setTeacherPassword(e.target.value)}
                            placeholder="****"
                            onKeyDown={e => e.key === 'Enter' && handleRevealAnswer()}
                        />
                        <button type="button" onClick={handleRevealAnswer} className="bg-[#1c3a60] hover:bg-[#346297] px-4 py-1.5 rounded-lg text-sm font-bold transition-colors">
                            Desbloquear
                        </button>
                    </div>
                )}
                    ...
                    {!showActivity ? (
                        <div className="space-y-6">
                            <div className="relative">
                                {data.type === 'guided_practice' && <span className="absolute -top-3 -right-3 z-10 bg-[#346297] text-white text-xs px-2 py-1 rounded font-bold shadow-sm">Teoría</span>}
                                {/* Use TheoryRenderer for interactive formats, fallback to PedagogicalWrapper */}
                                {(() => {
                                    if (effectiveType !== "text") {
                                        return (
                                            <div className="bg-[#1c3a60] rounded-2xl p-5 border border-[#346297] shadow-xl">
                                                <TheoryRenderer
                            ...
                            <div className="flex justify-between pt-4">
                                {currentChunkIndex > 0 ? (
                                    <button
                                        ...
                                        className="bg-[#cbe0f6] hover:bg-slate-300 text-[#346297] px-5 py-3 rounded-xl font-bold text-lg flex items-center gap-2 transition-all hover:scale-105 active:scale-95"
                                    >
                                        <ChevronLeft /> Atrás
                                    </button>
                                ) : <div />}
                                <button
                                    ...
                                    className="bg-[#1c3a60] hover:bg-[#1c3a60] text-white px-6 py-3 rounded-xl font-bold text-lg shadow-lg shadow-teal-200 flex items-center gap-2 transition-all hover:scale-105 active:scale-95"
                                >
                        ...
                        <div className="space-y-4">
                            <button
                                ...
                                className="flex items-center gap-2 text-[#1c3a60] hover:text-[#1c3a60] font-bold text-sm transition-colors"
                            >
            ...
            {/* Loading Overlay */}
            {isDownloading && (
                <div className="fixed inset-0 bg-[#1c3a60]/90 z-[9999] flex flex-col items-center justify-center backdrop-blur-sm">
                    <Sparkles className="w-16 h-16 text-[#73a4db] animate-spin mb-6" />
                    <h2 className="text-3xl font-bold text-white mb-2">Construyendo Documento...</h2>
                    <p className="text-[#73a4db] text-lg">Añadiendo historia y minijuegos. Esto tomará unos segundos.</p>
                </div>
            )}
            ...
            <div
                ...
                style={{ display: "none" }}
            >
                <h1 className="text-3xl font-bold text-center mb-6 text-[#1c3a60] border-b-2 border-teal-200 pb-4">{data.title}</h1>
                ...
                <div className="border-t-4 border-[#346297] pt-6 mt-8">
                    <h2 className="text-2xl font-bold text-[#1c3a60] mb-4">Actividad Práctica</h2>
                    {data.type === 'guided_practice' ? (
                        <div className="bg-[#f0f5fb] p-6 rounded-xl border border-[#cbe0f6]">
