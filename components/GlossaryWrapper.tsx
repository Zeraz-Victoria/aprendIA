                ...
                if (match) {
                    return (
                        <span key={index} className="relative group inline-block cursor-help z-10">
                            <span className="underline decoration-sky-400 decoration-wavy text-sky-700 dark:text-[#AD74C3] font-medium">
                                {part}
                            </span>
                            <span className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-64 p-3 bg-[#522566] dark:bg-[#EADFF0] text-white dark:text-[#522566] text-sm rounded-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 shadow-xl pointer-events-none text-left z-50">
                                <strong className="block mb-1 text-[#AD74C3] dark:text-[#522566] capitalize">{match.palabra}</strong>
                                {match.definicion}
