                ...
                if (match) {
                    return (
                        <span key={index} className="relative group inline-block cursor-help z-10">
                            <span className="underline decoration-sky-400 decoration-wavy text-sky-700 dark:text-[#2e9f6c] font-medium">
                                {part}
                            </span>
                            <span className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-64 p-3 bg-[#0a2d1d] dark:bg-[#c1ebd5] text-white dark:text-[#0a2d1d] text-sm rounded-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 shadow-xl pointer-events-none text-left z-50">
                                <strong className="block mb-1 text-[#2e9f6c] dark:text-[#0a2d1d] capitalize">{match.palabra}</strong>
                                {match.definicion}
