...
export default function StudentProfile({ onClose }: { onClose: () => void }) {
    ...
    const getFrameStyle = (frame: string | undefined | null) => {
        ...
        if (frame === 'frame_ice') return { borderColor: '#73a4db', boxShadow: '0 0 20px rgba(115, 164, 219,0.6)' };
        if (frame === 'frame_lightning') return { borderColor: '#346297', boxShadow: '0 0 20px rgba(52, 98, 151,0.6)' };
    ...
    return (
        <div className="bg-white rounded-3xl w-full max-w-2xl max-h-[90vh] overflow-hidden relative shadow-2xl flex flex-col"
            ...
            <button
                ...
                style={{ background: 'rgba(240, 245, 251,0.2)' }}
            >
            ...
            <div className="p-8 text-white relative overflow-hidden shrink-0"
                ...
                <div className="absolute top-[-20px] right-[80px] w-32 h-32 rounded-full opacity-10" style={{ background: '#73a4db' }} />
                ...
                <div className="flex items-center gap-6 relative z-10">
                    ...
                    <div>
                        ...
                        <p className="font-medium text-lg mt-1" style={{ color: '#cbe0f6' }}>Aventurero Matemático</p>
                    </div>
                </div>
                ...
                {isEditingAvatar && (
                    <div className="mt-8 p-4 rounded-2xl relative z-10 animate-slide-up"
                        ...
                        <div className="flex justify-between items-center mb-3">
                            ...
                            <button onClick={() => setIsEditingAvatar(false)} style={{ color: '#cbe0f6' }}>
                                <X className="w-4 h-4" />
                            </button>
                        </div>
                        ...
                        {/* Avatars */}
                        <div className="mb-4">
                            <h4 className="text-xs font-bold mb-2 uppercase tracking-wider" style={{ color: '#cbe0f6' }}>AVATARES</h4>
                        ...
                        {inventory[currentUser?.id || ""]?.some(id => id.startsWith('frame_')) && (
                            <div>
                                <h4 className="text-xs font-bold mb-2 uppercase tracking-wider" style={{ color: '#cbe0f6' }}>MARCOS DE PERFIL</h4>
                                <div className="flex gap-3">
                                    <button
                                        onClick={() => updateStudentFrame(null)}
                                        className="px-3 py-1.5 rounded-lg text-sm font-bold transition-colors"
                                        style={!currentUser?.activeFrame
                                            ? { background: 'white', color: '#1c3a60' }
                                    ...
                                    {inventory[currentUser?.id || ""]?.includes('frame_ice') && (
                                        <button
                                            ...
                                            style={currentUser?.activeFrame === 'frame_ice'
                                                ? { background: '#73a4db', color: 'white', boxShadow: '0 0 12px rgba(115, 164, 219,0.5)', borderColor: 'white' }
                                                : { background: 'rgba(255,255,255,0.1)', color: '#73a4db', borderColor: 'transparent' }}
                                        >
                                            ❄️ Hielo
                                        </button>
                                    )}
                                    ...
                                    {inventory[currentUser?.id || ""]?.includes('frame_lightning') && (
                                        <button
                                            ...
                                            style={currentUser?.activeFrame === 'frame_lightning'
                                                ? { background: '#346297', color: 'white', boxShadow: '0 0 12px rgba(52, 98, 151,0.5)', borderColor: 'white' }
                                                : { background: 'rgba(255,255,255,0.1)', color: '#cbe0f6', borderColor: 'transparent' }}
                                        >
                                            ⚡ Rayo
                                        </button>
                                    )}
            ...
            {/* Scrollable Content */}
            <div className="flex-1 overflow-y-auto w-full" style={{ background: '#f0f5fb' }}>
                
                {/* Grades Section */}
                <div className="px-8 pt-8">
                    <div className="flex items-center justify-between mb-6">
                        <div className="flex items-center gap-3">
                            <div className="p-2 rounded-xl" style={{ background: '#cbe0f6' }}>
                                <Medal className="w-6 h-6" style={{ color: '#1c3a60' }} />
                            </div>
                            <h3 className="text-xl font-bold" style={{ color: '#1c3a60' }}>Calificaciones</h3>
                        </div>
                        <div className="px-4 py-2 rounded-2xl flex flex-col items-end"
                            style={{ background: '#cbe0f6' }}>
                            <span className="text-[10px] font-bold uppercase tracking-wider" style={{ color: '#73a4db' }}>Promedio Global</span>
                    ...
                    {!currentUser?.automaticProjectGrades || currentUser.automaticProjectGrades.length === 0 ? (
                        <div className="text-center py-6 rounded-3xl border mb-6"
                            style={{ background: '#cbe0f6', borderColor: 'transparent' }}>
                            <p className="text-sm" style={{ color: '#73a4db' }}>Aún no tienes actividades calificadas.</p>
                        </div>
                    ) : (
                                ...
                                return (
                                    <div key={`${pg.worldId}-${idx}`} className="bg-white border-2 p-4 rounded-2xl flex justify-between items-center shadow-sm"
                                        ...
                                        <div className="min-w-0 flex-1">
                                            <h4 className="font-bold text-sm truncate" title={worldTitle} style={{ color: '#1c3a60' }}>{worldTitle}</h4>
                                            <p className="text-[10px] uppercase font-black" style={{ color: '#73a4db' }}>Automático</p>
                                        </div>
                ...
                <div className="p-8 pb-12">
                    <div className="flex items-center gap-3 mb-6">
                        <div className="p-2 rounded-xl" style={{ background: '#cbe0f6' }}>
                            <Medal className="w-6 h-6" style={{ color: '#346297' }} />
                        </div>
                        <h3 className="text-xl font-bold" style={{ color: '#1c3a60' }}>Tus Logros</h3>
                    </div>

                    {loading ? (
                        <div className="space-y-4">
                            {[1, 2, 3].map(i => (
                                <div key={i} className="h-20 rounded-2xl w-full animate-pulse" style={{ background: '#cbe0f6' }}></div>
                            ))}
                        </div>
                            ...
                            {achievements.map((ach) => (
                                <div key={ach.id} className="flex items-center gap-4 bg-white border p-4 rounded-2xl hover:shadow-md transition"
                                    ...
                                    <div className="flex-1">
                                        <h4 className="font-bold text-lg" style={{ color: '#1c3a60' }}>{ach.name}</h4>
                                        <p className="text-sm leading-snug" style={{ color: '#346297' }}>{ach.description}</p>
                                    </div>
                                    <div className="text-right flex flex-col items-end">
                                        <span className="text-xs font-bold mb-1" style={{ color: '#73a4db' }}>RECOMPENSA</span>
                        ...
                        <div className="text-center py-10 rounded-3xl border"
                            ...
                            <h4 className="font-bold mb-1" style={{ color: '#1c3a60' }}>Aún no tienes logros</h4>
                            <p className="text-sm" style={{ color: '#346297' }}>
