...
function FlashcardsView({ cards, accentColor }: { cards: { front: string; back: string }[]; accentColor: string }) {
    ...
    return (
        <div className="flex flex-col items-center gap-4 w-full">
            <div className="text-xs font-bold uppercase tracking-widest text-[#73a4db] flex items-center gap-2">
            ...
            <div className="flex items-center gap-3">
                <button
                    ...
                    className="p-2 rounded-full bg-[#346297] text-white disabled:opacity-30 hover:bg-slate-600 transition"
                >
                ...
                <div className="flex gap-1.5 flex-wrap justify-center max-w-[200px]">
                    {cards.map((_, i) => (
                        <div key={i} className={`w-2 h-2 rounded-full transition ${i === currentIndex ? 'bg-[#73a4db]' : 'bg-slate-600'}`} />
                    ))}
                </div>
            ...
            <div className="relative">
                ...
                <div className="space-y-3 pl-4 border-l-4 border-dashed" style={{ borderColor: `var(--tw-${accentColor}-500, #94a3b8)` }}>
                    {sections.map((section, sIdx) => (
                        <div key={sIdx} className="bg-[#1c3a60]/80 rounded-2xl p-4 border border-[#346297]/50 shadow">
                            ...
                            <ul className="space-y-1.5">
                                {section.items.map((item, iIdx) => (
                                    <li key={iIdx} className="text-[#73a4db] text-sm flex items-start gap-2">
                                        <span className="text-[#73a4db] mt-0.5">▸</span>
                                        <span>{item}</span>
                                    </li>
                                ))}
                            </ul>
        ...
        <div className="w-full">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {branches.map((branch, bIdx) => (
                    <div key={bIdx} className="bg-[#1c3a60]/80 rounded-2xl p-4 border border-[#346297]/50 shadow hover:scale-[1.02] transition-transform">
                        ...
                        <ul className="space-y-1.5 pl-2">
                            {branch.children.map((child, cIdx) => (
                                <li key={cIdx} className="text-[#73a4db] text-xs flex items-start gap-2">
            ...
            <div className="space-y-0">
                {steps.map((step, i) => (
                    <div key={i} className="flex gap-4 items-start">
                        ...
                        <div className="bg-[#1c3a60]/80 rounded-2xl p-4 border border-[#346297]/50 flex-1 mb-3 shadow">
                            <h4 className={`font-bold text-${accentColor}-300 text-sm mb-1`}>{step.heading}</h4>
                            <p className="text-[#73a4db] text-xs leading-relaxed">{step.text}</p>
                        </div>
                    </div>
                ))}
            </div>
    ...
    return (
        <div className="w-full">
            ...
            <div className="space-y-3">
                {words.map((word, idx) => (
                    <div key={idx} className="bg-[#1c3a60]/80 rounded-2xl p-4 border border-[#346297]/50 shadow">
                        <p className="text-[#73a4db] text-xs mb-2 font-medium">
                                ...
                                <div className="flex flex-wrap gap-1">
                                    {word.palabra.split("").map((_, cIdx) => (
                                        <input
                                            ...
                                            inputMode="text"
                                            autoComplete="off"
                                            className="w-8 h-8 sm:w-9 sm:h-9 rounded-lg bg-[#346297] border border-[#346297] text-white text-center font-bold text-sm uppercase focus:border-yellow-400 focus:ring-1 focus:ring-yellow-400 outline-none transition"
