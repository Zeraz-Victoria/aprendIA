...
export default function TeacherToolkit({ students, classroomId }: any) {
    ...
    return (
        <div className="p-6">
            <h2 className="text-2xl font-bold text-[#522566] mb-6">Herramientas de Clase (Toolkit)</h2>
            
            <div className="grid grid-cols-3 gap-4 mb-8 max-w-2xl mx-auto">
                <button 
                    onClick={() => setActiveTool(activeTool === 'random' ? null : 'random')}
                    className={`p-6 rounded-2xl flex flex-col items-center gap-3 transition-all border-2 ${activeTool === 'random' ? 'bg-[#522566] border-[#AD74C3] text-white shadow-xl scale-105' : 'bg-white border-[#EADFF0] text-[#522566] hover:border-[#AD74C3]'}`}
                >
                ...
                <button 
                    ...
                    className={`p-6 rounded-2xl flex flex-col items-center gap-3 transition-all border-2 ${activeTool === 'timer' ? 'bg-[#522566] border-[#AD74C3] text-white shadow-xl scale-105' : 'bg-white border-[#EADFF0] text-[#522566] hover:border-[#AD74C3]'}`}
                >
                ...
                <button 
                    ...
                    className={`p-6 rounded-2xl flex flex-col items-center gap-3 transition-all border-2 ${activeTool === 'noise' ? 'bg-[#522566] border-[#AD74C3] text-white shadow-xl scale-105' : 'bg-white border-[#EADFF0] text-[#522566] hover:border-[#AD74C3]'}`}
                >
            ...
            <div className="max-w-2xl mx-auto">
                {activeTool === 'random' && (
                    <div className="bg-white rounded-3xl p-10 flex flex-col items-center border-2 border-[#EADFF0] shadow-xl animate-fade-in text-center">
                        <div className={`w-40 h-40 rounded-full flex items-center justify-center text-8xl mb-6 bg-[#F8EDFB] border-4 border-[#EADFF0] transition-all duration-300 ${isSpinning ? 'animate-spin scale-110 shadow-2xl shadow-[#AD74C3]/50' : 'shadow-inner'}`}>
                            {randomStudent?.avatar || '❓'}
                        </div>
                        <h3 className="text-3xl font-black text-[#522566] mb-8 min-h-[40px]">
                            {randomStudent ? randomStudent.name : '¿Quién será?'}
                        </h3>
                        <button 
                            onClick={handlePickRandom}
                            disabled={isSpinning || visibleStudents.length === 0}
                            className="bg-[#522566] hover:bg-[#7A3A8E] disabled:opacity-50 text-white px-8 py-4 rounded-2xl font-black uppercase tracking-widest transition-all hover:scale-105 active:scale-95 shadow-lg shadow-[#522566]/30 flex items-center gap-2"
                        >
                ...
                {activeTool === 'timer' && (
                    <div className="bg-white rounded-3xl p-10 flex flex-col items-center border-2 border-[#EADFF0] shadow-xl animate-fade-in">
                        <div className={`text-9xl font-black mb-8 font-mono tabular-nums tracking-tighter ${time <= 10 && time > 0 ? 'text-rose-500 animate-pulse' : time === 0 ? 'text-rose-600' : 'text-[#522566]'}`}>
                            {formatTime(time)}
                        </div>
                        ...
                        <div className="flex items-center gap-3 bg-[#F8EDFB] p-2 rounded-xl">
                            <input 
                                type="number" 
                                value={inputMinutes} 
                                onChange={(e) => setInputMinutes(e.target.value)}
                                className="w-16 bg-white border border-[#EADFF0] rounded-lg text-center font-bold text-[#522566] py-1"
                                min="1" max="60"
                            />
                            <span className="text-xs font-bold text-[#AD74C3] uppercase tracking-widest pr-3">Minutos</span>
                            <button onClick={() => { setTime(parseInt(inputMinutes) * 60 || 0); setIsTimerRunning(false); }} className="bg-[#EADFF0] hover:bg-[#AD74C3] hover:text-white text-[#522566] text-[10px] font-black uppercase tracking-widest px-3 py-1.5 rounded-lg transition-colors">
                                Establecer
                            </button>
                        </div>
                    </div>
                ...
                {activeTool === 'noise' && (
                    <div className="bg-white rounded-3xl p-10 flex flex-col items-center border-2 border-[#EADFF0] shadow-xl animate-fade-in text-center">
                        <div className="mb-6 relative w-64 h-64 flex items-center justify-center">
                            {/* Medidor visual */}
                            <div className="absolute inset-0 rounded-full border-8 border-[#F8EDFB]"></div>
                            <div 
                        ...
                        <h3 className="text-xl font-bold text-[#522566]">Medidor de Ruido</h3>
                        <p className="text-sm text-[#AD74C3] mt-2">Pide silencio si el nivel sube demasiado.</p>
                        {noiseLevel > 80 && (
